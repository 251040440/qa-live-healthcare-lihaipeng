...
项目同时提供了 Windows 平台的应用生命周期管理脚本：

### 脚本列表

| 脚本文件 | 功能描述 | 使用方法 |
|---------|---------|---------|
| `start.bat` | 启动应用服务 | `start.bat` 或双击运行 |
| `stop.bat` | 停止应用服务 | `stop.bat` 或双击运行 |
| `restart.bat` | 重启应用服务 | `restart.bat` 或双击运行 |
| `status.bat` | 查看应用状态 | `status.bat` 或双击运行 |
